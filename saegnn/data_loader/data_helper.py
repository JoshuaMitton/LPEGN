...
#             print(int(graph_meta[1]))
            ...
            curr_graph = torch.from_numpy(noramlize_graph(curr_graph)).permute(2,0,1)
#             print(f'curr_graph shape : {curr_graph.shape}')
            A = torch.sum(curr_graph, dim=0)
            A[A!=0] = 1
            edge_index, _ = dense_to_sparse(A)
            edge_index_dense = torch.ones(curr_graph.shape[-1], curr_graph.shape[-1]).nonzero().t()
            edge_attr = curr_graph.permute(1,2,0)
            row, col = edge_index
            edge_attr = edge_attr[row, col]
#             print(f'edge_index : {edge_index.shape}')
#             print(f'edge_attr : {edge_attr.shape}')
    #         x, edge_index, edge_attr, batch_idx = DenseToSparse(x["2"], A, x["2"])
            xin = torch.diagonal(curr_graph, dim1=-2, dim2=-1)
            xin = xin.permute(1, 0)
#             print(f'xin shape : {xin.shape}')
            
            graphs.append(Data(T2=edge_attr, T1=None, T0=None, edge_index=edge_index, edge_index_dense=edge_index_dense, edge_attr=edge_attr, y=int(graph_meta[1]), num_nodes=xin.shape[0]))
#             graphs.append(Data(x2=xin, x1=None, x0=None, edge_index=edge_index, edge_index_dense=edge_index_dense, edge_attr=edge_attr, y=int(graph_meta[1]), num_nodes=xin.shape[0]))
